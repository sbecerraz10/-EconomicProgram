...
public class AVLTreeST<Key extends Comparable<Key>, Value> extends BST<Key, Value> {
    ...
    public Iterable<Key> keysLevelOrder() throws InterruptedException {
        ...
        if (!isEmpty()) {
            ...
            while (!queue2.isEmpty()) {
				...
				try {
					x = queue2.poll();
				} catch (structures.NoSuchElementException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
                queue.offer((Key) x.getKey());
                if (x.getLeft() != null) {
                    queue2.offer((Node<Key, Value>) x.getLeft());
                }
                if (x.getRigth() != null) {
                    queue2.offer((Node<Key, Value>) x.getRigth());
                }
            }
        }
        return (Iterable<Key>) queue;
    ...
    private void keys(Node<Key, Value> x, Queue<Key> queue, Key lo, Key hi) {
        ...
        if (cmplo <= 0 && cmphi >= 0) queue.offer((Key) x.getKey());
